...
def process_hazard_deduplication(device_id, timestamp, location, hazard, radius_meters=Config.DEDUPLICATION_RADIUS_METERS):
    ...
    if matched_incident:
        ...
        cursor.execute("""
            UPDATE incidents 
            SET report_count = ?, confidence = ?, priority_score = ?, last_updated_at = ?
            WHERE id = ?
        """, (new_count, new_confidence, new_priority, now_iso, incident_id))
        ...
        cursor.execute("""
            INSERT INTO incidents (incident_code, hazard_type, latitude, longitude, priority_score, confidence, report_count, first_reported_at, last_updated_at)
            VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)
        """, (inc_code, hazard_type, lat, lng, initial_priority, confidence, now_iso, now_iso))

        incident_id = cursor.lastrowid
        action = "CREATED"

    # Store full Edge AI detection log record
    cursor.execute("""
        INSERT INTO detection_logs (incident_id, device_id, raw_class, bbox, latitude, longitude, confidence, speed_kmh, timestamp)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (incident_id, device_id, raw_class, bbox, lat, lng, confidence, speed_kmh, timestamp))

...
def process_hazard_deduplication(
    ...
    # Remember whether the matched incident was already verified to prevent duplicate alerts
    was_verified = (
    ...
    if matched_incident:
        ...
        # Retain existing image if incoming duplicate report lacks one
        final_image = image_data if image_data else matched_incident.get("image_data")
    ...
    # Store the complete Edge AI detection log with snapshot
    cursor.execute(
        """
    ...
    verification_status = evaluate_incident_verification(incident_id)

    # Create an authority alert only when newly verified
    alert = None

...
export default function Dashboard() {
  ...
  const handleStatusChange = async (incidentId, newStatus) => {
    try {
      ...
      setIncidents(prev =>
        prev.map(item => (item.incident_id === incidentId ? { ...item, status: newStatus } : item))
      );
      if (detailedIncident && detailedIncident.incident_id === incidentId) {
        setDetailedIncident(prev => ({ ...prev, status: newStatus }));
      }
  ...
  const filteredIncidents = useMemo(() => {
    return incidents.filter(inc => {
      ...
      if (filters.search.trim() !== '') {
        ...
        const matchesBus = inc.bus_id?.toLowerCase().includes(query);
        const matchesType = inc.type?.toLowerCase().includes(query);
        const matchesLocation = inc.location_name?.toLowerCase().includes(query);
        if (!matchesId && !matchesBus && !matchesType && !matchesLocation) return false;
      }
      return true;
    });
  }, [incidents, filters]);
  ...
  return (
    <div className="dashboard-container">
      ...
      <header className="dashboard-header">
        <div className="header-brand">
          ...
          <div className="brand-text">
            <h1>PATHA SARATHI</h1>
            <span className="brand-subtitle">GIS Road Hazard & Incident Command Center</span>
          </div>
        </div>

        <div className="header-controls">
          <div className="live-status-indicator">
            <Radio size={14} className="pulse-icon text-emerald" />
            <span>LIVE SYNC</span>
            {lastRefreshed && <span className="refresh-time">({lastRefreshed})</span>}
          </div>
      ...
      <main className="dashboard-main">
        ...
        <div className="mobile-view-tabs" role="tablist" aria-label="Dashboard Views">
          <button
            type="button"
            role="tab"
          ...
          >
            <List size={16} /> Incident List ({filteredIncidents.length})
          </button>
        </div>

        {/* Desktop Split Layout / Responsive Layout */}
        <div className="dashboard-grid">
          {/* Left Column: Filter Panel & Incident List */}
          <div className={`dashboard-sidebar ${mobileTab === 'list' ? 'mobile-visible' : ''}`}>
            <FilterPanel
              filters={filters}
            ...
            <div className="incident-list-container">
              <div className="list-header">
                ...
                <span className="count-pill">{filteredIncidents.length} items</span>
              </div>

              {loading ? (
                <div className="list-loading">
                  <RefreshCw size={24} className="spin text-accent" />
                  <p>Loading GIS hazard reports...</p>
                </div>
              ) : filteredIncidents.length === 0 ? (
                <div className="list-empty">
                  <Compass size={32} className="text-muted" />
                  <p>No incidents match the selected filter criteria.</p>
                  <button type="button" className="btn-inline" onClick={handleResetFilters}>
                    Clear Filters
                  </button>
                </div>
              ) : (
                  ...
                  {filteredIncidents.map((incident) => (
                    <IncidentCard
                      ...
                      onSelect={(inc) => {
                        ...
                        // On mobile, switch to map view when card clicked so user sees the marker center
          ...
          {/* Right Column: GIS Map View */}
          <div className={`dashboard-map-area ${mobileTab === 'map' ? 'mobile-visible' : ''}`}>
            <Map
              incidents={filteredIncidents}
              selectedIncident={selectedIncident}
              onSelectIncident={(inc) => setSelectedIncident(inc)}
              onViewDetails={(inc) => setDetailedIncident(inc)}
            />
          </div>
        </div>
      </main>
